                            ...
                            <div key={index} className="intro-y" onClick={() => setSelectedList(contract)}>
                                <div className={twMerge([
                                    ...
                                    <div className="ml-4 mr-auto">
                                        <div className="font-medium">No.Póliza/Contrato:  {contract.CodigoPoliza && contract.CodigoPoliza !== "" ? contract.CodigoPoliza : contract.CodigoSolicitud}</div>
                                        <div className="text-slate-500 text-xs mt-0.5 flex gap-1">
                                            <p className="font-medium">{t("insuranceDNI")}:</p>  {contract.DNIAsegurado}
                                        </div>
                                        <div className="text-slate-500 text-xs mt-0.5  flex gap-1">
                                            <p className="font-medium">Fecha Alta:</p>{contract?.FechaAltaSolicitud ? moment(contract?.FechaAltaSolicitud).format('YYYY-MM-DD HH:mm') : '---'}
                                        </div>
                                        <div className="text-slate-500 text-xs mt-0.5  flex gap-1">
                                            <p className="font-medium">Fecha Estado:</p>{contract?.FechaUltimaModif ? moment(contract?.FechaUltimaModif).format('YYYY-MM-DD HH:mm') : '---'}
                                        </div>
                                        <div className="text-slate-500 text-xs mt-0.5  flex gap-1">
                                            <p className="font-medium">CCC:</p>{contract?.CCC ?? '---'}
                                        </div>
                                        <div className="text-slate-500 text-xs mt-0.5  flex gap-1">
                                            <p className="font-medium">Anulado sin efecto:</p>{contract?.AnuladoSE ? 'SI' : 'NO'}
                                        </div>
                                        <div className="text-slate-500 text-xs mt-0.5  flex gap-1">
                                            <p className="font-medium">{t("branch")}:</p>{contract.Ramo.Codigo}
                                        </div>
                                        <div className="text-slate-500 text-xs mt-0.5  flex gap-1">
