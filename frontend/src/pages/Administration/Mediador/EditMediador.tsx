...
const EditMediador = ({ show, setShow, onSubmit, selectedRow }: Props) => {
   ...
   const updateProfileSchema = (selectedRow: any) =>
      yup.object().shape({
   ...
   const handleSubmit = async () => {
      ...
      const [error, response] = await handlePromise(
         selectedRow?.MediadorId
            ? MediadorService.updateMediator(selectedRow.MediadorId, toSend)
            : MediadorService.createMediator(toSend),
      );
      setLoading(false);
      if (!response.ok) {
         return setAlert({
   ...
   return (
      ...
      >
         <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 auto-cols-max w-full justify-center">
            <InputField control={control} name="Nombre" label="name" placeholder="Nombre ..." />

            <InputField control={control} name="Codigo" label="code" placeholder="Código ..." />

            <InputField control={control} name="Canal" label="channel" placeholder="Canal ..." />

            <InputField control={control} name="Zona" label="zone" placeholder="Zona ..." />
            ...
            <InputField control={control} name="Email" label="email" placeholder="Correo..." />

            <InputField control={control} name="Responsable" label="responsible" placeholder="Responsable ..." />
            ...
            <InputField control={control} name="Responsable2" label="responsible2" placeholder="Responsable 2 ..." />
            ...
            <CheckBoxField control={control} name="Reclamar" label="claim" />

            <CheckBoxField control={control} name="Activo" label="active" />
         </div>
      </ParentModal>
   );
};
