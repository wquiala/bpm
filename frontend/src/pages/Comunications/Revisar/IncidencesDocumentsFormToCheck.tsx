...
const IncidencesDocumentsFormToCheck = ({ incidencesDocuments, setIncidencesDocuments }: Props) => {
   ...
   const onSubmit = async (data: any) => {
      ...
      for (const inci of data.incidences) {
         const toSend = {
            Revisada: inci.revisada,
            Enviar: inci.enviada,
            DocumentoId: inci.DocumentoId,
         };

         const [error, response] = await handlePromise(
            DocumentIncidenceService.updateDocumentIncidence(inci.IncidenciaDocId, toSend),
         );
