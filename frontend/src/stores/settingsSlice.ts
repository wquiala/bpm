...
export const getCompanyList = createAsyncThunk('settings/companies', async (_, thunkAPI) => {
   const [error, response, data] = await handlePromise(CompanyService.getCompanyList());
   if (!response.ok) {
...
interface SettingsState {
   company: { CompaniaId: string; Nombre: string; Codigo: string; Producto: any[]; Mediador: any[] } | null;
   companyList: { CompaniaId: string; Nombre: string; Codigo: string; Producto: any[]; Mediador: any[] }[];
   caja: string | null;
   lote: string | null;
}
